...
double *SimpleExcitation::getBuffer(int numOfSamples)
{
	...
	//memcpy(gensamplebuffer, tmpBuf, sizeof(double)*numOfSamples); // this would be used if we plan not to have interpolations at all. it's fast
...
// the equivalent of this is in the AudioGeneratorOut class now (:
...
void SimpleExcitation::computeBandLimitImpulse() {
	if(useSinc) {
		...
		// generate second sinc as low pass sinc and then invert it into a high pass [to have opposite of what we need]
		sum = 0;
		for(int i=0; i<M+1; i++) {
			if (i!=M/2)
