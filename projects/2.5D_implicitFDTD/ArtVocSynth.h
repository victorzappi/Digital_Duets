...
public:
	...
	ArtVocSynth();
	~ArtVocSynth();
	void init(string shaderLocation, int *domainSize, int *excitationCoord, int *excitationDimensions, float ***domain, int audioRate, int rateMul,
			  int periodSize, float ***widthMap, float openSpaceWidth, float exLevel=1, float magnifier=1, int *listCoord=NULL, float exLpF=22050, bool useSinc=true);
	void init(string shaderLocation, int *domainSize, int audioRate, int rateMul, int periodSize, float ***widthMap,
			  float openSpaceWidth, float exLevel=1, float magnifier=1, int *listCoord=NULL, float exLpF=22050, bool useSinc=true);
	...
	void setAbsorptionCoefficient(float alpha);
	float alphaFromAdmittance(float mu);
...
private:
	...
	void initExcitation(int rate_mul, float exLevel, float normExLpF, bool useSinc=true);
};
...
inline void ArtVocSynth::setAbsorptionCoefficient(float alpha) {
	vocalTractFDTD.setAbsorptionCoefficient(alpha);
}
inline float ArtVocSynth::alphaFromAdmittance(float mu) {
	return VocalTractFDTD::alphaFromAdmittance(mu);
}
