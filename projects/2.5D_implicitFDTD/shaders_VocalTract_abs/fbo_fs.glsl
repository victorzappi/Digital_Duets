...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// P(n+1)  = [ width_c*P(n) - r*c^2*dt * (width_x*Vx(n) - width_x_left*Vx_left(n) + width_y*Vy(n) - width_y__down*Vy_down(n) ) / ds ] / width_c*[ 1 + ( 1 - b + s) * dt ]
//
...
vec4 computeFDTD() {
	...
	vec4 w_c = texture(widthTexture, tex_c).rgba;
	vec4 w_r = texture(widthTexture, tex_r).rgba;
	vec4 w_u = texture(widthTexture, tex_u).rgba;
	
	vec3 width_x   = vec3(w_c.x, w_r.x, w_u.x);
	vec3 width_y   = vec3(w_c.y, w_r.y, w_u.y);
	vec3 width_x_l = vec3(w_c.z, w_r.z, w_u.z);
	vec3 width_y_d = vec3(w_c.w, w_r.w, w_u.w);
	...
	vec3 width_c = (width_x+width_x_l+width_y+width_y_d)/4;
	...
	// update pressure
	vec3 p_next = (p*width_c - bulk_dt_by_dx * (vx*width_x - vx_left*width_x_l + vy*width_y - vy_down*width_y_d)) / (width_c*(1 + sigma_prime_dt));
	//@VIC dirichlet
...
vec4 saveAudio() {
	...
	if( (diffx  < deltaCoordX) && (diffx >= 0) &&
		...
		//retColor[int(writeChannel)] = audio; //VIC//@VIC
	}
	return retColor;
}
