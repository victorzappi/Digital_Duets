...
void ArtVocSynth::init(string shaderLocation, int *domainSize, int *excitationCoord, int *excitationDimensions, float ***domain, int audioRate,
		               int rateMul, int period_size, float ***widthMap, float openSpaceWidth, float exLevel, float magnifier, int *listCoord, float exLpF, bool useSinc) {
	...
	simulationRate  = vocalTractFDTD.init(shaderLocation, domainSize, excitationCoord, excitationDimensions, domain,
										  audioRate, rateMul, periodSize, widthMap, openSpaceWidth, magnifier, listCoord, exLpF);

	initExcitation(rateMul, exLevel, exLpF, useSinc);
}

void ArtVocSynth::init(string shaderLocation, int *domainSize, int audioRate, int rateMul, int period_size, float ***widthMap,
					   float openSpaceWidth, float exLevel, float magnifier, int *listCoord, float exLpF, bool useSinc) {
	// normExLpF is passed both to the vocal tract and the subglottal excitation
	// in both cases it serves to calculate the low pass cut off frequency of the filter that assures
	// that we are injecting in the simulation too high frequencies
	//
	...
	rate            = audioRate;
	rateMultiplier  = (rateMul>0) ? rateMul : 1; // juuust in case
	simulationRate  = vocalTractFDTD.init(shaderLocation, domainSize, audioRate, rateMul, period_size, widthMap,
									      openSpaceWidth, magnifier, listCoord, exLpF);

	initExcitation(rateMul, exLevel, exLpF, useSinc);
}

void ArtVocSynth::initExcitation(int rate_mul, float excitationLevel, float normExLpF, bool useSinc) {
	excitation = new SimpleExcitation(excitationLevel, rate, periodSize*rate_mul, simulationRate, normExLpF, useSinc); // periodSize*rate_mul cos we have to provide enough samples per each simulation step
	...
	audiofile.init(filename, rate, periodSize, excitationLevel);
}
