...
void HDH_AudioEngine::render(float sampleRate, int numOfSamples, int numOutChannels, double **framebufferOut, int numInChannels, double **framebufferIn) {
	...
	if(numOfAudioModulesInOut==1) {
		...
		//memcpy(framebufferOut[1], /*framebufferOut[2]*/framebufferOut[0], sizeof(double) * numOfSamples);

	#ifndef SCARLETT_HEADPHONES
		memcpy(framebufferOut[0], outBuff, sizeof(double) * numOfSamples);
		memcpy(framebufferOut[1], framebufferOut[2], sizeof(double) * numOfSamples);
	#else
		memcpy(framebufferOut[2], outBuff, sizeof(double) * numOfSamples);
		memcpy(framebufferOut[3], framebufferOut[2], sizeof(double) * numOfSamples);
	#endif


	#ifdef SCARLETT_DOUBLE_HEADPHONES
		memcpy(framebufferOut[4], framebufferOut[2], sizeof(double) * numOfSamples);
		memcpy(framebufferOut[5], framebufferOut[2], sizeof(double) * numOfSamples);
	#endif
	...
	#ifndef SCARLETT_HEADPHONES
		outbuff = framebufferOut[0];
	#else
		outBuff = framebufferOut[2];
	#endif
