...
//----------------------------------------
// control variables, externs from main.cpp
//---------------------------------------
...
const float maxDampFactor = 0.05;
const float minDampFactor = 0;

extern float areaPropagationFactor[AREA_NUM];
double areaDeltaPropFactor[AREA_NUM];// = areaPropagationFactor[0]/100;
const float maxPropFactor = 0.5;
const float minPropFactor = 0.0001;
...
/*	if(area==-1)
	...
	if(nextD <minDampFactor)
		nextD = minDampFactor;
	else if(nextD>maxDampFactor)
		nextD = maxDampFactor;
	...
	//if(update) {
		...
		printf("Area %d damping factor: %f\n", areaIndex, (nextD-minDampFactor)/(maxDampFactor-minDampFactor));
	//}
	//printf("Area %d damping factor: %f\n", area, nextD);
...
void changeAreaPropagationFactor(float deltaP/*, bool update, int area*/) {
	...
	if(nextP<minPropFactor)
		nextP = minPropFactor;
	else if(nextP>maxPropFactor)
		nextP = maxPropFactor;
	...
	//if(update) {
		...
		printf("Area %d propagation factor: %f\n", areaIndex, (nextP-minPropFactor)/(maxPropFactor-minPropFactor));
	//}
	//printf("Area %d propagation factor: %f\n", area, nextP);
}
...
/*		setAreaDampingFactor(i, frame->damp[i]);
	 ...
	 * dampAreaExcitation(touchedArea); // stop excitation
				...
				//areaExciteOnHold[touchedArea] = false; // hold will be released when more down, when the trigger is handled...
...
inline void modulateAreaDampPress(double press, int area) {
	...
	float nextD = areaDampFactor[area]+delta;
	if(nextD <0)
		nextD = 0;
	...
	for(int touch : preFingersArea_touches[index]) {
		...
		if(is_preFingerPress_dynamic[touch]) {
			...
			if(preFingersTouch_press[touch]==press_prop) {
				...
				printf("Area %d damp factor on hold!\n", index);
...
bool assignPreFinger(int slot, int area, int *coord=NULL) {
	...
	if(preFingersTouch_area[slot] == -1 && postFingerTouch_area[slot]==-1) {
		...
		if(nextPreFingerMotion_dynamic_wait || nextPreFingerPress_dynamic_wait) {
			...
			//printf("boundAreaListener_touch[%d]: %d\n", area, boundAreaListener_touch[area]);
...
inline bool checkPreFinger(int slot, int area, int *coord=NULL/*, bool set=false*/) {
...
void handleNewTouch(int currentSlot, int touchedArea, int coord[2]) {
	...
	// continuous excitation, which needs extra care for areas on hold
		...
		if(areaExciteOnHold[touchedArea]) {
			...
			if(/*listenerDist*/computeAreaListDist(touchedArea, coord[0], coord[1])<MIN_DIST && postFingerArea_touch[touchedArea]==-1 && !touchIsInhibited[currentSlot]) {
			...
			else if(/*exciteDist*/computeAreaExcitetDist(touchedArea, coord[0], coord[1])<MIN_DIST && (keepHoldArea_touch[touchedArea]==-1)) {
			...
			/*if(checkPreFinger(currentSlot, touchedArea, coord, true)) {
				modifierActionsPreFinger(currentSlot, touchedArea, coord[0], coord[1], (float)pressure[currentSlot]);
				return;
			}*/
			// now check assign fingers [including the listener we might have just added]
			/*if(assignPreFinger(currentSlot, touchedArea, coord)) { //VIC here there is no need for controlling if this is new touch, because we get here only if it is a new touch [if area is on hold, HandleNewTouch() is not called by HandleTouchDrag()!]
				printf("touch %d is new pre finger for area %d\n", currentSlot, touchedArea);
				return;
			}*/
			// otherwise we release all the excitations that were stuck there and GO ON with other checks [we actually release hold during those further checks]
				...
				//areaExciteOnHold[touchedArea] = false; // hold will be released more down, when the trigger is handled...
			}
		}
		// if not on hold, we do the routine assignment of pre fingers, if any
		/*else if( newTouch[currentSlot] && assignPreFinger(currentSlot, touchedArea, coord) )
			printf("touch %d is new pre finger for area %d\n", currentSlot, touchedArea);*/

		// then we move on with the usual logic for continous controllers
...
*/
	...
	if(triggerTouch_area[currentSlot] != -1) {
		...
		if( checkPercussive(area) || !areaExciteOnHold[area]) {
