...
SimpleExcitation::SimpleExcitation(float maxLevel, unsigned int audio_rate, unsigned short period_size, unsigned int simulationRate,
	...
	fileImpulse.init(filename, simulation_rate, maxLevel, periodSize);
...
SimpleExcitation::~SimpleExcitation() {
...
double *SimpleExcitation::getBuffer(int numOfSamples) {
	...
	//memcpy(gensamplebuffer, tmpBuf, sizeof(double)*numOfSamples); // this would be used if we plan not to have sample based controls at all. it's fast
...
/*float *SimpleExcitation::getBufferFloat(int numOfSamples)
{
	...
	aliasingFilter.process(tmpBuf, numOfSamples);
	memcpy(gensamplebuffer, tmpBuf, sizeof(double)*numOfSamples);

	excitationBuffer.d = gensamplebuffer;

	// union saves memory but std::copy should be faster
	for(int n = 0; n<numOfSamples; n++)
		excitationBuffer.f[n] = gensamplebuffer[n];

	return excitationBuffer.f;

	// here we use float buffer and copy()
	std::copy( getBuffer(numOfSamples), gensamplebuffer + numOfSamples, floatsamplebuffer);

	return floatsamplebuffer;
}*/

void SimpleExcitation::setFreq(double f) {
	freq = f;
...
void SimpleExcitation::computeBandLimitImpulse() {
	if(useSinc) {
		...
		// and do spectral inversion for band pass [this is what we need]
		for(int i=0; i<M+1; i++) {
		...
		for(int i=0; i<M+1; i++)
			sinc_windowed_impulse[i] *= amp; // now we're ready to go!
