...
vec4 two_mass_excitation(){
	...
	vec4 prev_out_old;  //ug/A1, x1, x2, A1 at N-2
	// feedback
	...
	// Using Equation 18 from IF72
		// Compute Force on Mass-1
	...
	//Changed the sign of the second term from Kees' Implementation
	...
	//Calculate new x1 and x2 values by solving equation A-9 from S87
	x1_next = (a22*b1 - a12*b2) / det;
	x2_next = (a11*b2 - a21*b1) / det;


	//Calculate New areas from new x1 and x2 displacement values and advance ug
	...
	if (Ag1<0 || Ag2<0) {
		...
		//Rtot changed to Ag2 instead of Ag1 in first term
		float Rtot = ( rho / 2)*(0.37 / (Ag2*Ag2) + (1 - 2 * (Ag2 /  A1_next)*(1 - Ag2 /  A1_next)) / (Ag2*Ag2))
		*abs(ug)
