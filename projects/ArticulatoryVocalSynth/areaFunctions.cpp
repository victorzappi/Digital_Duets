...
int createStraightTube(float length, float width, float ds, int domainSize[], float ***domainPixels, int &widthPixel, bool open) {
	int xPixels = round(length/ds)+1; // to close the tube in the back
	...
	float wallVal   = ImplicitFDTD::getPixelAlphaValue(ImplicitFDTD::cell_wall);
	if( (yPixels%2)!=0 ) {
		ImplicitFDTD::fillTextureRow( excitationCoord[1]+halfWidth,	excitationCoord[0], excitationCoord[0]+xPixels-1, wallVal, domainPixels);
		ImplicitFDTD::fillTextureRow( excitationCoord[1]-halfWidth, 	excitationCoord[0], excitationCoord[0]+xPixels-1, wallVal, domainPixels);
		// close excitation side
		if(!open) {
			ImplicitFDTD::fillTextureColumn( excitationCoord[0],  excitationCoord[1]-halfWidth, excitationCoord[1]-1,         wallVal, domainPixels);
			ImplicitFDTD::fillTextureColumn( excitationCoord[0],  excitationCoord[1]+1,         excitationCoord[1]+halfWidth,  wallVal, domainPixels);
		}
	}
	else {
		ImplicitFDTD::fillTextureRow( excitationCoord[1]+halfWidth+1,	excitationCoord[0], excitationCoord[0]+xPixels-1, wallVal, domainPixels);
		ImplicitFDTD::fillTextureRow( excitationCoord[1]-halfWidth, 		excitationCoord[0], excitationCoord[0]+xPixels-1, wallVal, domainPixels);
		// close excitation side
		if(!open) {
			ImplicitFDTD::fillTextureColumn( excitationCoord[0],  excitationCoord[1]-halfWidth, excitationCoord[1]-1,           wallVal, domainPixels);
			ImplicitFDTD::fillTextureColumn( excitationCoord[0],  excitationCoord[1]+1,         excitationCoord[1]+halfWidth+1, wallVal, domainPixels);
		}
