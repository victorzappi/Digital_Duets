...
uniform vec2 typeValues[cell_numOFTypes]; // 11 = ImplicitFDTD::cell_numOFTypes
...
vec4 computeFDTD() {
	...
	data = getCellStateData(inOutTexture, tex_r, cellTypes.y);
	beta[1]           = data[0];
	sigma_prime_dt[1] = data[1];
	
	// upper neighbour cell
	data = getCellStateData(inOutTexture, tex_u, cellTypes.z);
	beta[2]           = data[0];
	...
	float alpha = 0.004;
	float z_inv = 1 / (rho_c*( (1+sqrt(1-alpha))/(1-sqrt(1-alpha)) )); 
	
	bvec2 is_wall_bool = bvec2(beta.y != 1, beta.z != 1);
