...
// UI
// set once
uniform float uiAlphaMin;
uniform float uiAlphaMax;
const int UI_AREA_BUTTONS_NUM = 6; // must be same value as in FlatLandFDTD.h
const int UI_AREA_SLIDERS_NUM = 8; // must be same value as in FlatLandFDTD.h
uniform bool hideUI;

uniform vec4 areaButton[UI_AREA_BUTTONS_NUM]; // set once
uniform vec4 areaSlider[UI_AREA_SLIDERS_NUM]; // set once
uniform float areaButTrig[UI_AREA_BUTTONS_NUM];
uniform float areaSlidVal[UI_AREA_SLIDERS_NUM];
...
const vec4 color_danger    = vec4(128/256.0, 0/256.0, 0/256.0, 1);
...
void main () {
	...
	if( (cellType == cell_air) || (cellType >= cell_pml0 && cellType <= cell_pml5) || (cellType == cell_dead) ) {
	...
	if(!hideUI) {
		...
		for(int i=0; i<UI_AREA_BUTTONS_NUM; i++) {
			if( (coord.x>=areaButton[i].x) && (coord.x<areaButton[i].x+areaButton[i].z)  &&
			     (coord.y>=areaButton[i].y) && (coord.y<areaButton[i].y+areaButton[i].w) )
				frag_color = mix(frag_color, /*color_blue*/color_orange, areaButTrig[i]); // trigger oscillated between uiAlphaMin and uiAlphaMax
		}
	
		// UI area sldiers
		for(int i=0; i<UI_AREA_SLIDERS_NUM; i++) {
			if( (coord.x>=areaSlider[i].x) && (coord.x<areaSlider[i].x+areaSlider[i].z)  &&
			     (coord.y>=areaSlider[i].y) && (coord.y<areaSlider[i].y+areaSlider[i].w) ) {
				if(coord.y <= areaSlidVal[i])
					frag_color = mix(frag_color, /*color_orange*/color_yellow, uiAlphaMax); 
				else
					frag_color = mix(frag_color, /*color_orange*/color_yellow, uiAlphaMin); 
			}
		}
	}
