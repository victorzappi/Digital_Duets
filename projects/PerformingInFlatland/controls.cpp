...
bool startUIinteraction(float xpos, float ypos) {
	...
	if(triggered>-1) {
		buttonsMapping(triggered + UI_AREA_BUTTONS_NUM*(state==1));
		return true;
	}
...
void key_callback(GLFWwindow *, int key, int scancode, int action, int mods) {
	if(action == GLFW_PRESS || action == GLFW_REPEAT) {
		...
		if(action != GLFW_REPEAT) {
			switch(key) {
				...
				case GLFW_KEY_DELETE:
					...
					if(shiftIsPressed) {
						switchExcitationModel(exMod_noFeedback);
						freq = 15000 * (exp(0.60835*9)-1)/freq_exp_max;
						updateFrequency();
					}
				...
				case GLFW_KEY_SPACE:
					spaceBarIsPressed = true;//slowMotion(true);
					break;
		...
		switch(key) {
			...
			case GLFW_KEY_SPACE:
				spaceBarIsPressed = false;//slowMotion(false);
				break;
