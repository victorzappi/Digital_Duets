...
// public methods
//------------------------------------------------------------------------------------------
DemoFDTD::DemoFDTD() : DemoTextureFilter(){
	numOfStates = 2;
    ...
    listenUnifName = "listenerFragCoord"; // we define the name of the first listener, so that parent class retrieves correct uniform location in DemoTextureFilter::init(). The second one is hardcoded in initAdditionalUniforms()
...
int DemoFDTD::init(string shaderLocation, int *domainSize, int *excitationCoord, int *excitationDimensions,
	...
	excitationCellNum = excitationDimensions[0]*excitationDimensions[1]; // before DemoTextureFilter::init(), cos this value is used inside initAdditionaUniforms(), that is called from DemoTextureFilter::init()
	int sim_rate = DemoTextureFilter::init(domainSize, numOfStates, excitationCoord, excitationDimensions, domain, audioRate, rateMul, periodSize, mag,
...
int DemoFDTD::init(string shaderLocation, int *domainSize, int audioRate, int rateMul, unsigned int periodSize, float mag, int *listCoord,
		       unsigned short inChannels, unsigned short inChnOffset, unsigned short outChannels, unsigned short outChnOffset) {
	preInit(shaderLocation, listCoord);
	int sim_rate = DemoTextureFilter::init(domainSize, numOfStates, audioRate, rateMul, periodSize, mag,
			                           inChannels, inChnOffset, outChannels, outChnOffset);
	postInit();
...
int DemoFDTD::setDomain(int *excitationCoord, int *excitationDimensions, float ***domain) {
	excitationCellNum = excitationDimensions[0]*excitationDimensions[1]; // before DemoTextureFilter::init(), cos this value is used inside initAdditionaUniforms(), that is called from DemoTextureFilter::seteDomain()

	if(DemoTextureFilter::setDomain(excitationCoord, excitationDimensions, domain) != 0)
		return 1;
...
/*float *DemoFDTD::getBufferFloat(int numSamples, double *inputBuffer) {
...
double **DemoFDTD::getFrameBuffer(int numOfSamples, double **input) {
	// handle domain changes
	updatePixelDrawer();
...
void DemoFDTD::getListenerPosition(int &x, int &y) {
...
int DemoFDTD::shiftListenerV(int delta) {
...
int DemoFDTD::shiftListenerH(int delta){
...
int DemoFDTD::setListenerPosition(int x, int y) {
...
int DemoFDTD::setMaxPressure(float maxP) {
	if(inited) {
		maxPressure = maxP;
		updateMaxPress = true;
		return 0;
...
int DemoFDTD::fromRawWindowToDomain(int *coords) {
	// XD
...
int DemoFDTD::fromDomainToGlWindow(int *coords) {
...
int DemoFDTD::fromDomainToTexture(int *coords) {
	if(coords[0]<-domainShift[0] || coords[0]>domain_width+domainShift[0]-1  || coords[1]<-domainShift[1] || coords[1]>domain_height+domainShift[1]-1)
		return 1;
...
// here we go from the domain [what the user uses] to the full texture frame of reference
int DemoFDTD::fromTextureToDomain(int *coords) {
...
int DemoFDTD::setPhysicalParameters(void *params) {
	if(!inited)
		return -1;
...
int DemoFDTD::setCell(float type, int x, int y) {
	if(!inited)
		return -1;
...
int DemoFDTD::resetCell(int x, int y) {
	if(!inited)
		return -1;
...
int DemoFDTD::setMousePosition(int x, int y) {
	if(!inited)
		return -1;
...
float DemoFDTD::getPixelAlphaValue(int type, int PMLlayer, int dynamicIndex, int numOfPMLlayers){
	float retVal;
	switch(type) {
...
void DemoFDTD::fillTexturePixel(int x, int y, float value, float ***pixels) {
	pixels[x][y][0] = 0;
	pixels[x][y][1] = 0;
...
void DemoFDTD::fillTextureRow(int index, int start, int end, float value, float ***pixels) {
...
void DemoFDTD::fillTextureColumn(int index, int start, int end, float value, float ***pixels) {
...
void DemoFDTD::preInit(string shaderLocation, int *listCoord) {
	 ...
	 *  2 for texture pos (x, y) of this cell at current state -> needed for DemoFDTD explicit stencil
	 *  2 for texture pos (x, y) of left neighbor cell at current state -> needed for DemoFDTD explicit stencil
	 *  2 for texture pos (x, y) of up neighbor cell at current state -> needed for DemoFDTD explicit stencil
	 *  2 for texture pos (x, y) of right neighbor cell at current state -> needed for DemoFDTD explicit stencil
	 *  2 for texture pos (x, y) of down neighbor cell at current state -> needed for DemoFDTD explicit stencil
	 *
...
void DemoFDTD::postInit() {
	minDs           = ds;
	slowMo_sleep_ns = slowMo_stretch/simulation_rate;
	frameDuration   = FRAME_DURATION;
}

void DemoFDTD::initPhysicalParameters() {
    c = C_AIR;
    printf("\n");
    printf("\tc: \t%f\n", c);
    computeDeltaS();
...
void DemoFDTD::allocateResources() {
...
void DemoFDTD::initImage(int domainWidth, int domainHeigth) {
...
void DemoFDTD::initVao(GLuint vbo, GLuint shader_program_fbo, GLuint shader_program_render, GLuint &vao) {
...
void DemoFDTD::initFrame(int *excitationCoord, int *excitationDimensions, float ***domain) {
	//-----------------------------------------------------
	...
	for(int i = 0; i <domain_width; i++) {
		for(int j = 0; j <domain_height; j++) {
			...
			// to clarify, this is needed for the standard structure of the example DemoFDTD,
			// where:
...
void DemoFDTD::fillImage(float ***pixels) {
...
int DemoFDTD::initAdditionalUniforms() {
	//--------------------------------------------------------------------------------------------
	// FBO uniforms
	//--------------------------------------------------------------------------------------------
...
int DemoFDTD::computeListenerPos(int x, int y) {
	int coords[2] = {x, y};
...
void DemoFDTD::computeNextListenerFrag() {
...
// automatically called by DemoTextureFilter::initUniforms()
...
int DemoFDTD::computeListenerFrag(int x, int y) {
...
void DemoFDTD::computeSamples(int numSamples, double *inputBuffer) {
	...
	for(int n = 0; n < numSamples; n++) {
		...
		for(int i=0; i<rate_mul; i++) {
			...
			// this is what we do:
			// 0_save audio     ---> in particular we want to read audio from listener position in state x-1, COS WE ARE SURE IT'S READY [DemoFDTD for x will be computed later]; this explains the weird backwards mapping of the listener coordinates among the state quads [see setUpListener()]
			// 1_compute DemoFDTD   ---> solve for state x. To solve we need excitation cell. To be sure that no one is still messing up with it while solving for state x, we read excitation from state x-1: this explains step 0 AND the weird backwards mapping of the excitation coordinates among the state quads [see fillVertices()]
...
void DemoFDTD::renderWindowInner() {
	//------------------
...
void DemoFDTD::getSamples(int numSamples) {
	if(!slowMotion) {
		//---------------------------------------------------------------------------------------------------------------------------
		// get data
		//---------------------------------------------------------------------------------------------------------------------------
...
void DemoFDTD::updatePixelDrawer() {
	if(resetPixels) {
		pixelDrawer->reset();
		resetPixels = false;
	}
...
void DemoFDTD::updateUniformsRender() {
	if(updateListener) {
		setUpListenerRender();
		updateListener = false;
	}
...
float DemoFDTD::computeDs(float _dt, float _c) {
	return _dt * _c * sqrt( (double)2.0 );
}
int DemoFDTD::initPixelDrawer() {
...
int DemoFDTD::computeMouseFrag(int x, int y) {
	int coords[2] = {x, y};
...
void DemoFDTD::readAllTextureDebug() {
	pbo.unbind();
	pbo_allTexture.bind();
...
bool DemoFDTD::computePMLcoeff(float * &coeff) {
	if(numOfPML<2) {
...
float DemoFDTD::getPixelBlueValue(int type) {
