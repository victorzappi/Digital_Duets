/*
 * DemoTextureFilter.cpp
 *
 *  Created on: 2015-09-23
 *      Author: Victor Zappi
 */

#include "DemoTextureFilter.h"
...
DemoTextureFilter::DemoTextureFilter(){
	domain_width		= -1;
	domain_height		= -1;
	gl_width            = -1;
	gl_height           = -1;
...
int DemoTextureFilter::init(int *domainSize, int numOfStates, int *excitationCoord, int *excitationDimensions,
...
int DemoTextureFilter::init(int *domainSize, int numOfStates, int audioRate, int rateMul, unsigned int periodSize, float mag,
		                unsigned short inChannels, unsigned short inChnOffset, unsigned short outChannels, unsigned short outChnOffset) {
	AudioModuleInOut::init(periodSize, inChannels, inChnOffset, outChannels, outChnOffset);
...
int DemoTextureFilter::setDomain(int *excitationCoord, int *excitationDimensions, float ***domain) {
	if(!needDomain)
		return -1;
...
GLFWwindow *DemoTextureFilter::getWindow(int *size) {
	if(!inited)
		return NULL;
...
// protected
//------------------------------------------------------------------------------------------------------------------------------------------------
void DemoTextureFilter::initSimulationParameters(int audioRate, int rateMul) {
...
// helper functions
//---------------------------------
bool _demo_gl_log_err (const char* message, ...) {
  va_list argptr;
...
void _demo_glfw_error_callback (int error, const char* description) {
	_demo_gl_log_err ("GLFW ERROR: code %i msg: %s\n", error, description);
}
//---------------------------------
...
int DemoTextureFilter::initWindow(int *domainSize, int numOfStates, int periodSize, float mag) {
	...
	// let's start this!
#ifndef VISUAL_DEBUG
	window = DemoTextureFilter::initOpenGL(gl_width*magnifier, gl_height*magnifier, name, NULL/*, fullscreen, monitorIndex*/);
#else
	window = DemoTextureFilter::initOpenGL(textureWidth*magnifier, textureHeight*magnifier, name, NULL/*, fullscreen, monitorIndex*/);
#endif
...
int DemoTextureFilter::initShaders() {
...
void DemoTextureFilter::initGLDataStruct() {
	//-----------------------------------------------------------
...
void DemoTextureFilter::initTexture(int *excitationCoord, int *excitationDimensions, float ***domain) {
	//--------------------------------------------------------
...
int DemoTextureFilter::initUniforms() {
	//--------------------------------------------------------
	// Uniforms
	//--------------------------------------------------------
...
// quick and dirty
void demo_glfw_init_callback(int val, const char *name) {
...
GLFWwindow *DemoTextureFilter::initOpenGL(int width, int height, string windowName, GLFWwindowsizefun cbfun/*, bool fullScreen, int monIndex*/) {
	assert (restart_gl_log ());

	// start GL context and O/S window using the GLFW helper library
	gl_log ("starting GLFW\n%s\n", glfwGetVersionString());
	// register the error call-back function that we wrote, above
	glfwSetErrorCallback(_demo_glfw_error_callback);

	// let's set an error callback, just in case...
	glfwSetErrorCallback(demo_glfw_init_callback);
...
bool DemoTextureFilter::loadShaderProgram(const char *vertShaderPath, const char *fragShaderPath, GLuint &vs, GLuint &fs, GLuint &shader_program)
{
...
void DemoTextureFilter::initVbo(int size, float *vertices, GLuint &vbo) {
	glGenBuffers (1, &vbo);
	glBindBuffer (GL_ARRAY_BUFFER, vbo); // means that we work on this VBO
	glBufferData (GL_ARRAY_BUFFER, size, vertices, GL_STATIC_DRAW);
}

/*void DemoTextureFilter::loadTexture() {
...
void DemoTextureFilter::completeInit() {
	//--------------------------------------------------------
	// Ready to go!
	//--------------------------------------------------------
	...
	shaderFbo.use();
	checkError("DemoTextureFilter::init() -> glUseProgram");
	glBindFramebuffer(GL_FRAMEBUFFER, fbo); // Render to our framebuffer
}


void DemoTextureFilter::setUpListener() {
	for (int i =0; i<numOfStates; i++)
...
string DemoTextureFilter::readFile(const char *filePath) {
    string content;
...
bool DemoTextureFilter::restart_gl_log () {
...
bool DemoTextureFilter::gl_log (const char* message, ...) {
  va_list argptr;
  FILE* file = fopen (GL_LOG_FILE, "a");
  if (!file) {
    fprintf (
...
void DemoTextureFilter::log_gl_params () {
  GLenum params[] = {
...
void DemoTextureFilter::_print_shader_info_log (GLuint shader_index) {
  int max_length = 2048;
  int actual_length = 0;
  char log[2048];
...
bool DemoTextureFilter::checkCompileErrors(GLuint shader_index)
{
...
void DemoTextureFilter::_print_programme_info_log (GLuint program) {
  int max_length = 2048;
  int actual_length = 0;
  char log[2048];
...
bool DemoTextureFilter::checkLinkErrors(GLuint program)
{
...
const char* DemoTextureFilter::GL_type_to_string (GLenum type) {
  switch (type) {
...
void DemoTextureFilter::print_all (GLuint program) {
...
bool DemoTextureFilter::is_valid (GLuint program) {
...
void DemoTextureFilter::_update_fps_counter (GLFWwindow* window) {
...
void DemoTextureFilter::checkError(string lastCall)
{
