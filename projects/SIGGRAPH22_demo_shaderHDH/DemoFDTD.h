...
class DemoFDTD : public DemoTextureFilter {
public:
...
inline GLFWwindow *DemoFDTD::getWindow(int *size) {
	if(pixelDrawer == NULL)
		return NULL;

	return DemoTextureFilter::getWindow(size);
}

inline int DemoFDTD::fromRawWindowToGlWindow(float *coords) {
	// XD
...
// private
//----------------------------------------------------------
inline int DemoFDTD::getNumOfQuads() {
	return quad_numOfQuads;
}

inline void DemoFDTD::updatePixelDrawerCritical() {
...
inline void DemoFDTD::updateUniformsCritical() {
...
inline void DemoFDTD::setUpListenerRender() {
...
inline void DemoFDTD::setUpExCellNum() {
...
inline void DemoFDTD::setUpExCellNumRender() {
	if(excitationCellNum > 0)
...
inline void DemoFDTD::setUpMaxPressure() {
...
inline void DemoFDTD::setUpPhysicalParams() {
	float c2_dt2_invDs2 = c*c*dt*dt/(ds*ds);
	glUniform1f(c2_dt2_invDs2_loc, c2_dt2_invDs2);
	//checkError("glUniform1f c2_dt2_invDs2");
}

inline void DemoFDTD::setUpMouseRender() {
	glUniform2f(mouseCoord_loc_render, mouseFragCoord[0], mouseFragCoord[1]);
}

inline float DemoFDTD::getDS() {
	return ds;
}

inline void DemoFDTD::setSlowMotion(bool slowMo) {
	slowMotion  = slowMo;
	slowMo_time = 0;
	slowMo_step = 0;
}

inline void DemoFDTD::setAllWallBetas(float wallBeta) {
...
inline float DemoFDTD::estimateDs(float rate, int rateMul, float c) {
	return computeDs(1/(rate*rateMul), c);
}

inline int DemoFDTD::saveFrame() {
	if(inited && (pixelDrawer != NULL))
...
inline int DemoFDTD::saveFrameSequence() {
	if(inited && (pixelDrawer != NULL))
...
inline int DemoFDTD::openFrame(string filename) {
	if(inited && (pixelDrawer != NULL)) {
...
inline int DemoFDTD::loadFrame(frame *f) {
	if(inited && (pixelDrawer != NULL)) {
...
inline int DemoFDTD::openFrame(frame *f) {
	if(inited && (pixelDrawer != NULL)) {
...
inline int DemoFDTD::computeFrame(string filename, frame &f){
	if(inited && (pixelDrawer != NULL)) {
		if(&f != NULL)
...
inline int DemoFDTD::openFrameSequence(string filename, bool isBinary) {
	if(inited && (pixelDrawer != NULL)) {
...
inline int DemoFDTD::computeFrameSequence(string foldername) {
	if(inited && (pixelDrawer != NULL))
