...
//----------------------------------------
int domainSize[2] = {60, 60}; //{380, 380}
float magnifier   = 5;
int rate_mul      = 1;
...
int initDrumSynth(float ***domainPixels) {
	...
	// init
	drumSynth->init(shaderLocation, domainSize, rate, rate_mul, period_size, magnifier, listenerCoord);
...
int parseArguments() {
	...
	while (1) {
		int c;
		if ((c = getopt(at_data.argc, at_data.argv, "hD:d:r:f:b:p:m:s:vneCM:qt:R:T:")) < 0)
			break;
		switch (c) {
			case 'h':
				morehelp++;
...
int initAudioThread() {
	...
	audioEngine.addAudioModule(drumSynth);
