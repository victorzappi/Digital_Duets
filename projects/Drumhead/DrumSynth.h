...
enum drumEx_type {drumEx_sandbox, drumEx_const, drumEx_sin, drumEx_square, drumEx_noise,
					 drumEx_impTrain, drumEx_file, drumEx_windowedImp, drumEx_fileImp, drumEx_oscBank};


// this is a synth, with inside another synth that is filtered by the FDTD
class DrumSynth: public AudioGeneratorOut {
public:
	DrumSynth(bool useGpu);
	void init(string shaderLocation, int *domainSize, int audioRate, int rateMul, int periodSize, float magnifier, int *listCoord=NULL);
...
inline void DrumSynth::retrigger() {
}
#endif /* DRUMSYNTH_H_ */
