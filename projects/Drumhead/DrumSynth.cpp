...
void DrumSynth::init(string shaderLocation, int *domainSize, int audioRate, int rateMul, int period_size, float magnifier, int *listCoord) {
...
void DrumSynth::initExcitation(int rate_mul) {
	float maxExcitationLevel = 1;
	excitation = new DrumExcitation(maxExcitationLevel, rate, periodSize*rate_mul, simulationRate); // periodSize*rate_mul cos we have to provide enough samples per each simulation step
}
