...
//----------------------------------------
// domain variables, all externs in defineDomain.cpp and some in control.cpp too
//----------------------------------------
int domainSize[2] = {80, 80}; //{380, 380}
float magnifier   = 5;
int rate_mul      = 1;
...
//----------------------------------------
int listenerCoord[2]   = {(domainSize[0]/2)+3, (domainSize[1]/2)};
...
float dampFactor       = 0.001;
float propagationFactor = 0.5; // will call this rho, it is related to CFL num...as follows
float clampFactor      = 0;
float lowPassFreq      = 22000.0;
bool render            = true;
bool useGPU            = true;
bool timeTest          = false;
...
int period_size = 64;     		// period length in frames
int buffer_size = 128;		    // ring buffer length in frames [contains period]
...
static void help(void) {
	...
	printf(
			...
			"-M, 		GPU window magnifier value [flaoting point]\n"
...
int initDrumSynth(float ***domainPixels) {
	string shaderLocation = getCurrentFilePath() + "/shaders_Drumhead";

	// first check if fullscreen, must be done before init!
	//drumSynth->setFullscreen(fullscreen, monitorIndex);

	// init
	drumSynth->init(shaderLocation, domainSize, rate, rate_mul, period_size, excitationLevel, magnifier, listenerCoord);
	if(drumSynth->setDomain(excitationCoord, excitationDim, domainPixels) != 0)
	 return 1;

	// apply settings
	drumSynth->setExcitationVolume(excitationVol);
	drumSynth->setSubGlotExType(drumExType);
	drumSynth->setDampingFactor(dampFactor);
	drumSynth->setPropagationFactor(propagationFactor);
	drumSynth->setClampingFactor(clampFactor);
	drumSynth->setExcitationLowPassFreq(lowPassFreq);
...
int initSimulation() {
	...
	// can draw your pattern here...
/*	DrumheadGPU::fillTextureRow(0, 0, domainSize[1]-1, DrumheadGPU::cell_wall, domainPixels);
	DrumheadGPU::fillTextureRow(domainSize[0]-1, 0, domainSize[1]-1, DrumheadGPU::cell_wall, domainPixels);
	DrumheadGPU::fillTextureColumn(0, 0, domainSize[0]-1, DrumheadGPU::cell_wall, domainPixels);
	DrumheadGPU::fillTextureColumn(domainSize[1]-1, 0, domainSize[0]-1, DrumheadGPU::cell_wall, domainPixels);*/


	//DrumheadGPU::fillTextureRow(2, 1, 2, DrumheadGPU::cell_wall, domainPixels);

/*	DrumheadGPU::fillTextureRow(11, 9, 11, DrumheadGPU::cell_wall, domainPixels);
	DrumheadGPU::fillTextureRow(9, 9, 11, DrumheadGPU::cell_wall, domainPixels);
	DrumheadGPU::fillTextureColumn(9, 9, 11, DrumheadGPU::cell_dead, domainPixels);
	DrumheadGPU::fillTextureColumn(11, 9, 11, DrumheadGPU::cell_dead, domainPixels);*/
...
int parseArguments() {
	...
	while (1) {
		int c;
		if ((c = getopt(at_data.argc, at_data.argv, "hD:d:r:c:f:b:p:m:o:s:vneCM:qt:R:T:")) < 0)
			break;
		switch (c) {
			case 'h':
				morehelp++;
...
int initAudioThread() {
	...
	audioEngine.addGenerator(drumSynth);
...
int main(int argc, char *argv[]) {
	...
	drumSynth = new DrumSynth(useGPU); // needs to be created for both threads' sake

	if(!render)
		drumSynth->disableRender();
	...
	if(timeTest) {
		...
		for(int n=0; n<numOfTestRuns; n++) {
			...
			for(int i=0; i<numOfCycles; i++) {
				if(hitCnt++ == hitDeltaCycles) {
					hitCnt = 0;
					drumSynth->setSubGlotExType(drumEx_windowedImp);
				}
				drumSynth->getBufferFloat(period_size);
			}
		...
		for(int n=0; n<numOfTestRuns; n++) {
			...
			for(int i=0; i<numOfCycles; i++) {
				...
				drumSynth->getBufferFloatReadTime(period_size, readTime);
		...
		for(int n=0; n<numOfTestRuns; n++) {
			...
			for(int i=0; i<numOfCycles; i++)
				drumSynth->getBufferFloatNoSampleRead(period_size);
			gettimeofday(&end_, NULL);
			elapsedTime = ( (end_.tv_sec*1000000+end_.tv_usec) - (start_.tv_sec*1000000+start_.tv_usec) );
		}
