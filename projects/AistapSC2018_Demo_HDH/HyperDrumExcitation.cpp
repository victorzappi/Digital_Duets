...
HyperDrumExcitation::HyperDrumExcitation(string folder, float maxLevel, unsigned int audio_rate, unsigned short period_size, unsigned int simulationRate) {
	AudioGeneratorInOut::init(period_size);
...
/*	if(listFilesInFolder(folder) !=0 )
	...
	numOfComponents = numOfFiles + numOfWaveforms + numOfInputs; // + impulse, sine, square, noise + 4 passthrough for 4 input channels
	...
	for(int i=0; i<numOfFiles; i++) {
		...
		if(chL!=string::npos) {
			audiofiles[i].init(maxLevel, fileNameAndPath, period_size, simulation_rate, 0);
			printf(", left channel only");
		}
		else if(chR!=string::npos) {
			audiofiles[i].init(maxLevel, fileNameAndPath, period_size, simulation_rate, 1);
			printf(", right channel only");
		}
		else {
			audiofiles[i].init(maxLevel, fileNameAndPath, period_size, simulation_rate);
	...
	// oscillators
	sine.init(simulation_rate, periodSize, osc_sin_, maxLevel, freq, 0.0);
	square.init(simulation_rate, periodSize, osc_square_, maxLevel, freq, 0.0);
	noise.init(simulation_rate, periodSize, osc_whiteNoise_, maxLevel, 0.0, 0.0);
	...
	for(int i=0; i<numOfInputs; i++) {
		passthrough[i].setChannel(i);
		passthrough[i].init(period_size);
	}
	...
	for(int i=1; i<numOfFiles+1; i++) {
		audioModulesOut[i] = &audiofiles[i-1];
		componentsNames.push_back(fileNames[i-1]);
		componentIsPercussive[i] = fileIsPercussive[i-1];
	}
	audioModulesOut[numOfFiles+1] = &sine;
	componentsNames.push_back("Sine");
	componentIsPercussive[numOfFiles+1] = false;
	audioModulesOut[numOfFiles+2] = &square;
	componentsNames.push_back("Square");
	componentIsPercussive[numOfFiles+2] = false;
	audioModulesOut[numOfFiles+3] = &noise;
	componentsNames.push_back("Noise");
	componentIsPercussive[numOfFiles+3] = false;

	for(int i=0; i<numOfInputs; i++) {
		audioModulesInOut[i] = &passthrough[i];
		string name = "Audio Input " + to_string(i);
		componentsNames.push_back(name.c_str());
		componentIsPercussive[numOfFiles+numOfWaveforms+i] = false;
	}
...
double *HyperDrumExcitation::getBuffer(int numOfSamples, float **input) {
	// generic handle
	double *tmpBuf;
	...
	if(currentExcitationId < numOfComponents-numOfInputs)
		...
		tmpBuf = audioModulesInOut[currentExcitationId-numOfWaveforms-numOfFiles]->getBuffer(numOfSamples, input);

	if(currentExcitationId==0 || currentExcitationId>=numOfComponents-numOfInputs) {
