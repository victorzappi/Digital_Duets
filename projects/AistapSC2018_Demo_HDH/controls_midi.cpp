...
*/
		...
		// remote control mapping //VIC test this shit and then do OSC...and then FULL DUPLEX
		else {
			int area = chn-1;
			...
			if(type==kmmControlChange) {
				...
				if(cc==0) {
					...
					setAreaDampingFactorNorm(area, val/127.0);
				}
				else if(cc==4) {
					setAreaPropagationFactorNorm(area, val/127.0);
				}
				else if(cc==5) {
