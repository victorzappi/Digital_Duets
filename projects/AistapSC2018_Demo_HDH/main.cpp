...
//----------------------------------------
int domainSize[2] = {158, 88};//{94, 52};//m20 //{126, 70};//m15 //{158, 88};//m12 //{190, 106};//m10  //{238, 133};//m8 //{318, 178};//m6
float magnifier   = 6;
int rate_mul      = 1;
bool fullscreen   = false;
int monitorIndex  = 0; // 0 or -1 are primary monitor
//----------------------------------------
...
//----------------------------------------
int listenerCoord[2]   = {domainSize[0]/2, domainSize[1]/2};
int areaListenerCoord[AREA_NUM][2] = {-1, -1};
int excitationDim[2]   = {0, 0}; // no starting excitation
int excitationCoord[2] = {3+domainSize[0]/2, domainSize[1]/2};
int drumExId = 0;
...
float areaDampFactor[AREA_NUM] = {0.001};
...
int period_size = 128;//(perf==3 || forceDuplex) ? 128 : 32;     		// period length in frames
...
// location variables
//----------------------------------------
string exciteFileFolder = getCurrentFilePath() + "/audiofiles";
string presetFileFolder = getCurrentFilePath() + "/presets";
...
static void help(void) {
	...
	printf(
			...
			"-W, 		width of domain [also moves listener to center]\n"
			"-H, 		height of domain [also moves listener to center]\n"
			"-M, 		GPU window magnifier value [floating point]\n"
			"-m, 		index of monitor where window will spawn [primary monitor by default]\n"
			...
			"-F, 		force full duplex audio engine\n"
			"\n");
...
int initDrumSynth(float ***domainPixels) {
	...
	// first check if fullscreen, must be done before init, cause this effects the window!
	//hyperDrumSynth->setFullscreen(fullscreen, monitorIndex); //@VIC

	// init
	hyperDrumSynth->init(shaderLocation, domainSize, magnifier, monitorIndex, fullscreen, rate, rate_mul, period_size, excitationLevel, exciteFileFolder, listenerCoord); //@VIC
	...
	//hyperDrumSynth->setExcitationType((DrumExcitation::drumExType)drumExId);
...
int parseArguments() {
	...
	while (1) {
		int c;
		if ((c = getopt(at_data.argc, at_data.argv, "hD:d:C:r:c:f:b:p:m:o:W:H:vneM:q4S:F")) < 0)
			break;
		switch (c) {
			case 'h':
				morehelp++;
			...
			case 'C':
				...
				//audioEngine.setCardName(playbackCardName); // done anyway outside
				break;
			case 'r':
				rate = atoi(optarg);
			...
			case 'c':
				...
				//audioEngine.setChannelNum(playbackChannels);  // done anyway outside
				break;
			case 'b':
			...
			case 'o':
				...
				//audioEngine.setAudioFormat(playbackFormat); // done anyway outside
				break;
			case 'v':
				verbose = 1;
			...
			case 'W':
				...
				listenerCoord[0] = (domainSize[0]/2);
				break;
			case 'H':
			...
			case 'F':
				forceDuplex = true;
				printf("Forcing full duplex audio!");
				break;
		}
	}
...
int initAudioThread() {
	...
	audioEngine.setFullDuplex( (perf==3) || forceDuplex);
	...
	//audioEngine.addAudioOutput(hyperDrumSynth);
...
int main(int argc, char *argv[]) {
	...
	if(perf==1)
		...
		folderPrefix = "ginevra_";
	exciteFileFolder = folderPrefix+exciteFileFolder;
	presetFileFolder = folderPrefix+presetFileFolder;
