...
DrumExcitation::DrumExcitation(float maxLevel, unsigned int audio_rate, unsigned short period_size, unsigned int simulationRate) {
	...
	// special impulse
	fastBandLimitImpulse(excitorVolume);
	...
	audiofile[0].init(maxLevel*0.3, filename, periodSize, simulation_rate);
	...
	audioModulesOut[6] = &audiofile[0];
	audioModulesOut[7] = &audiofile[1];
	audioModulesOut[8] = &audiofile[2];
	audioModulesOut[9] = &audiofile[3];
	audioModulesOut[10] = &audiofile[4];
	audioModulesOut[11] = &audiofile[5];
...
/*	if(currentExcitationId==2)
		printf("samplebuffer[0] %f\n", samplebuffer[0]);*/
...
void DrumExcitation::setExcitationId(int id) {
	...
	if(id<EXCITATIONS_NUM) {
		currentExcitationId = id;
		useBank = (id == COMPONENTS_NUM);
	}
	else {
...
// private methods
//---------------------------------------------------------------------------------------------------------------------------------
void DrumExcitation::fastBandLimitImpulse(float level) {  //Cuomo
	//-------------------------------------
