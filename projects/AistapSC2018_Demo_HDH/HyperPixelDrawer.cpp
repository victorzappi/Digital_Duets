...
HyperPixelDrawer::HyperPixelDrawer(int w, int h, int wrChan, Shader &shader_fbo, float ***startFrame,
	...
	//VIC how about accessing directly then enum from the filter the class is associated with? via including the header of the filter
...
int HyperPixelDrawer::dumpFrame(hyperDrumheadFrame *frame, string filename) {
	...
	// turn crossfade excitation into static ones! this makes the preset more consistent
	for(int i=0; i<gl_width; i++) {
		//printf("\n");
		for(int j=0; j<gl_height; j++) {
			float type = frame->pixels[j*gl_width*writeChannels+i*writeChannels+writeChannels-1];
			//printf("%f ", type);
			if( (type==(float)HyperDrumhead::cell_excitationA) || (type==(float)HyperDrumhead::cell_excitationB) ) {
				frame->pixels[j*gl_width*writeChannels+i*writeChannels+writeChannels-1] = (float)HyperDrumhead::cell_excitation;
				printf("swapped %d %d\n", i, j);
			}
		}
	}
	...
	for(int i=0; i<gl_height; i++) {
		for(int j=0; j<gl_width; j++) {
			...
			float type = frame->pixels[i*writeChannels*gl_width+j*writeChannels+2];

			if( (type==(float)HyperDrumhead::cell_excitationA) || (type==(float)HyperDrumhead::cell_excitationB) ) {
				frame->pixels[i*writeChannels*gl_width+j*writeChannels+2] = (float)HyperDrumhead::cell_excitation;
				printf("swapped %d %d\n", i, j);
			}
		}
	}
...
// then percussive files
int HyperPixelDrawer::openFrame(string filename, hyperDrumheadFrame *&frame, bool reload) {
	// fill it from file
	...
	if(!reload) {
		...
		// partial load of frame
		if(!reloadFrame(frame))
			return 2;
	}
	return 0;
}
...
bool HyperPixelDrawer::reloadFrame(hyperDrumheadFrame *frame) {
	// check if we are opening a frame that has the same size as the current one!
	if( (frame->width != gl_width) || (frame->height != gl_height) ) {
		printf("Heck! Trying to load a frame with size different from current simulation domain!\n");
		return false;
	...
	for(int i=0; i<gl_height; i++) {
			...
			float loadType = frame->pixels[i*writeChannels*gl_width+j*writeChannels+2];
			float currentType = framePixels[i*writeChannels*gl_width+j*writeChannels+2];

			// always leave trigger touch
			if( currentType==(float)HyperDrumhead::cell_excitationA || currentType==(float)HyperDrumhead::cell_excitationB )
				continue;
			// put back what missing
			else if( loadType==(float)HyperDrumhead::cell_boundary || loadType==(float)HyperDrumhead::cell_excitation )
				memcpy(framePixels+i*writeChannels*gl_width+j*writeChannels, frame->pixels+i*writeChannels*gl_width+j*writeChannels, writeChannels*sizeof(float));
			// remove what added
			else if( loadType==(float)HyperDrumhead::cell_air && (currentType==(float)HyperDrumhead::cell_boundary || currentType==(float)HyperDrumhead::cell_excitation) )
				memcpy(framePixels+i*writeChannels*gl_width+j*writeChannels, frame->pixels+i*writeChannels*gl_width+j*writeChannels, writeChannels*sizeof(float));
		}
	}
	return true;
