...
extern char story_vowel;
extern int asymmetric;
extern int rate_mul;
...
void saveImpulseResponse(float audioSamples[], int numOfSamples) {
	string asymmetries[3] = {"", "_res", "_asym"};
	string outfilepath_ = string(1,story_vowel) + "_" + to_string(rate_mul) + asymmetries[asymmetric]+".wav";
...
void calculateFreqResponse(int numFFTaudioSamples, int periodSize, float refFromants[], int refFormantsNum) {
	// at this period size, how many buffers from the synth we need to get the requested number of bins?
	int numOfFullBuffers = ( (float)numFFTaudioSamples )/( (float)periodSize);
	int residue = numFFTaudioSamples%periodSize;
	...
	float audioSamples[numFFTaudioSamples];
	...
	// just in case period_size and numFFTaudioSamples are not integer multiples
	if(residue>0)
	...
	int numOfFFTsamples = ((float)numFFTaudioSamples)/2.0 + 1; // number of samples computed in fft
#ifdef SSE_SUPPORT
	...
	// ...
	calculateFFT(audioSamples, numFFTaudioSamples, fftSamples);
	...
	searchFormants(magnitude, frequency, numOfFFTsamples, refFromants, refFormantsNum);
...
/*	ofstream myfile;
	...
	// save audio
	saveImpulseResponse(audioSamples, numFFTaudioSamples);
}
