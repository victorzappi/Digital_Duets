...
HyperDrumhead::HyperDrumhead() {
	...
	numOfInputs = 10+1; //VIC: [bad] hardcoded value from USBest Technology SiS touch screen [+1 is mouse]
	...
	// area excitations
	areaExciteInput_loc = new GLint[numOfInputs];
...
int HyperDrumhead::hideAreaListener(int index) {
	...
	// equivalent of computeListenerFrag(), but hardcoded, no checks
	//----------------------------------------------------------------
	areaListenerCoord[index][0] = -numOfPML-numOfOuterCellLayers-1;
	areaListenerCoord[index][1] = -numOfPML-numOfOuterCellLayers-1;
...
void HyperDrumhead::computeSamplesAreas(int numSamples, float *inputBuffer, float **inputAreaBuffer) {
	...
	for(int n = !slowMotion*slowFrameCnt; n < !slowMotion*numSamples + slowMotion; n++) {
		if(slowMotion) {
			usleep(slowMo_sleep_ns);
			slowFrameCnt++;
			audioRowChannel = 0; // to avoid messing up with audio row filling when ending slow motion
...
void HyperDrumhead::renderWindow() {
