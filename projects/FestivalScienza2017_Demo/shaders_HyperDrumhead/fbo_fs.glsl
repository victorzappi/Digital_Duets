...
const int numOfInputs = 10; // hardcoded value from USBest Technology SiS touch screen
const int maxNumOfAreas  = 1;
...
uniform float areaExciteInput[numOfInputs]; // excite only specific area

// listeners
uniform vec2 listenerFragCoord[numOfStates];
uniform vec2 areaListenerFragCoord[maxNumOfAreas][numOfStates];

uniform vec2 areaListenerBFragCoord[maxNumOfAreas][numOfStates];
...
vec4 computeFDTD() {
	...
	int index = int(frag_c.y); // to access local mu, rho and excitation
	int area = 0;
	...
	p_neigh.x    = frag_l.x;
	beta_neigh.x = float( (frag_l.w==cell_air)||(frag_l.w==cell_excitation) );
	gamma_neigh.x = frag_l.z;
	
	// up
	vec4 frag_u  = texture(inOutTexture, tex_u);
	p_neigh.y    = frag_u.x;
	beta_neigh.y = float( (frag_u.w==cell_air)||(frag_u.w==cell_excitation) );
	gamma_neigh.y = frag_u.z;
	
	// right
	vec4 frag_r  = texture(inOutTexture, tex_r);
	p_neigh.z    = frag_r.x;
	beta_neigh.z = float( (frag_r.w==cell_air)||(frag_r.w==cell_excitation) );
	gamma_neigh.z = frag_r.z;

	// down
	vec4 frag_d  = texture(inOutTexture, tex_d);
	p_neigh.w    = frag_d.x;
	beta_neigh.w = float( (frag_d.w==cell_air)||(frag_d.w==cell_excitation) );
	...
	p_next += areaExciteInput[index] * is_excitation;
...
float sampleAudio(vec2 audioCoord) {	
	...
	return audioFrag.r * int(audioFrag.a==cell_air || audioFrag.a==cell_excitation) * int(audioFrag.x>-1); 
}
...
vec4 saveAudio() {
	...
	if( (diffx  < deltaCoordX) && (diffx >= 0) &&
		...
		for(int i=0; i<numOfAreas; i++) {
			audioA = sampleAudio(areaListenerFragCoord[i][readState]);
			audioB = sampleAudio(areaListenerBFragCoord[i][readState]);
